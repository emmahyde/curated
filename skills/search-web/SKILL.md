...
**Subagent hard stop:** Do not answer the research question in chat. Your conclusion belongs only inside `<work_dir>/corpus-summary.md`. In a `[search-web-subagent]` run, any final response other than a single absolute path to an existing `corpus-summary.md` is invalid, even if the substantive answer is good. You must perform the corpus pipeline, create/update the corpus file, append the Research Brief, validate the file, and then return only the path.
...
**If you are the dispatched subagent** (`[search-web-subagent]` in your prompt): the appended brief must contain the exact heading `## Research Brief` and a line beginning `**Confidence:**`. Do not merely summarize the answer in the final chat message; write the brief into `corpus-summary.md`.
...
Before sending that final line, validate the artifact: resolve the path to an absolute path, confirm the file exists, confirm it contains `## Research Brief`, and confirm it contains `**Confidence:**`. If any check fails, fix the corpus file before replying. The final reply itself must contain only the absolute path, with no markdown formatting or extra lines.
...
<!-- SLOW_UPDATE_START -->
For a `[search-web-subagent]` run, treat the output contract above as the top-level success criterion: run the full validation checklist in `## Output contract` before replying, and return only the absolute path even when the user asked a natural-language question. A well-formed research artifact plus the correct role-specific output beats a polished chat answer — if you feel tempted to answer immediately because the conclusion is obvious, complete the corpus artifact first.

When you are the dispatcher, do not accept or paraphrase a subagent's direct substantive answer as sufficient. Require and read the returned `corpus-summary.md`, then answer the user from that artifact.
<!-- SLOW_UPDATE_END -->
